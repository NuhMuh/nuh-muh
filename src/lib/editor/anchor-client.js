...
export function preprocessBody(html) {
  ...
  root.querySelectorAll(ANCHOR_TAGS.join(',')).forEach((el) => {
    ...
    // ★★빈 블록 제거 해제됨 (2026-08-23 마틴 판정, 안 3 — 전면 보존).
    //
    // [왜 지웠었나] 앵커 무결성 때문이라 믿어졌으나, 실측 조사 결과 빈 블록을
    //   살려도 서버 방어선 3규칙 어디에도 걸리지 않는다. 규칙1은 p를 안 보고,
    //   규칙2는 앵커 붙은 p를 깊이 무관 통과시키며, 규칙3은 ID 중복만 본다.
    //   유지 근거는 청소 목적뿐이었다.
    //
    // [왜 풀었나] 문단을 나누려 엔터를 두 번 치면 생기는 빈 문단이 저장 시 사라져
    //   "빈 줄이 씹힌다"는 최대 불편이 됐다. 빈 문단은 조판 의도이지 쓰레기가 아니다.
    //
    // [상한 없음] 부분 완화("몇 개까지")는 답 없는 규칙이 되므로 배제됐다.
    //   엔터 세 번이면 빈 문단 둘이 그대로 남는다.
    //
    // [붙여넣기 주의] 외부(Word·구글독스) 붙여넣기 시 빈 문단이 대량 유입될 수 있다.
    //   이전엔 이 단계가 청소했다. 붙여넣기 정화는 유지보수 트랙으로 분리됨.
    //
    // isEmptyBlock은 남겨둔다 — 붙여넣기 정화가 같은 판정을 재사용할 것이다.
  });
